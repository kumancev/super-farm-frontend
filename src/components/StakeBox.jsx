...
export default function StakeBox() {
    ...
    const stake = async() => {
        try {
            ...
            let tx = await daiContract.connect(signer).approve(superFarmContract.address, amount)
            provider.waitForTransaction(tx.hash)
            .then(async() => {
                tx = await superFarmContract.connect(signer).stake(amount)
            })
            return tx
        } catch (error) {
            alert(error)
    ...
    const unstake = async() => {
        try {
            let signer = provider.getSigner()
            let amount = ethers.utils.parseEther(transferAmount)
            let tx = await superFarmContract.connect(signer).unstake(amount)
            return tx
        } catch (error) {
            alert(error)
        }
